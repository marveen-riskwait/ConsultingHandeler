...
export const api = {
  ...
  sars: (status) => request(`/sars${status ? `?status=${status}` : ""}`),
  createSar: (cid, payload) => request(`/customers/${cid}/sars`, { method: "POST", body: payload }),
  updateSar: (id, payload) => request(`/sars/${id}`, { method: "PATCH", body: payload }),
  submitSarForApproval: (id) => request(`/sars/${id}/submit-for-approval`, { method: "POST" }),
  approveSar: (id) => request(`/sars/${id}/approve`, { method: "POST" }),
  rejectSar: (id, reason) => request(`/sars/${id}/reject`, { method: "POST", body: { reason } }),
  markSarSubmitted: (id) => request(`/sars/${id}/mark-submitted`, { method: "POST" }),
  sarExportUrl: (id) => `/api/sars/${id}/export.xml`,
  dualControl: (status) => request(`/dual-control${status ? `?status=${status}` : ""}`),
  approveDualControl: (id) => request(`/dual-control/${id}/approve`, { method: "POST" }),
  rejectDualControl: (id, reason) => request(`/dual-control/${id}/reject`, { method: "POST", body: { reason } }),
  dualControlPolicy: () => request("/dual-control/policy"),
  setDualControlPolicy: (action_type, enabled) => request("/dual-control/policy", { method: "PUT", body: { action_type, enabled } }),
  dataExportUrl: (cid) => `/api/customers/${cid}/data-export`,
